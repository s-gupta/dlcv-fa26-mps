...
from models import LinearRegressionClassifier, NearestNeighbor, LogisticRegressionClassifier

FLAGS = flags.FLAGS
flags.DEFINE_enum('classifier', 'knn', ['knn', 'linear', 'logistic'], 'What classifier to run.')
flags.DEFINE_enum('feature', 'raw', ['raw'], 'What features to use.')
...
def linear_regression():
    ...
    data_val = featurize(data_val, FLAGS.feature)
    
    for num_train in FLAGS.num_train:
        data_train, labels_train = get_mnist_dataset('train', num_train)
        data_train = featurize(data_train, FLAGS.feature)

        epochs = FLAGS.epochs
        for wt in FLAGS.wt:
            dir_name = f'{FLAGS.out_dir}/num-{num_train}_wt-{wt}'
            writer = SummaryWriter(dir_name, flush_secs=30)
            linear = LinearRegressionClassifier(data_train, labels_train, 
                                                reg_wt=wt)
            linear.train()
            ...
            logging.info(f'num_train: {num_train}, wt: {wt}, accuracy: {accuracy}')
    ...
    for num_train in FLAGS.num_train:
        ...
        for lr in FLAGS.lr:
            for wt in FLAGS.wt:
                ...
                writer = SummaryWriter(dir_name, flush_secs=30)
                linear = LogisticRegressionClassifier(data_train, labels_train, 
                                                      epochs=epochs, lr=lr, 
                                                      reg_wt=wt, writer=writer)
                linear.train()
                preds_val = linear.predict(data_val)
                accuracy = compute_accuracy(labels_val, preds_val)
                writer.add_scalar('val_accuracy', accuracy, epochs)
