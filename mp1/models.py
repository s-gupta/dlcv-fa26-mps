...
class LinearRegressionClassifier(object):
    def __init__(self, data, labels, reg_wt=3e-5):
        self.data = data
        self.labels = labels
        self.reg_wt = reg_wt
        self.w = np.zeros((self.data.shape[1], 10))

    def train(self):
        """Train the linear classifier using closed form solution"""
        n_train, d = self.data.shape
        # BEGIN YOUR CODE
        pass
        # END YOUR CODE
