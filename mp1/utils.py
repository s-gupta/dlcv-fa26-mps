...
def corrupt(labels, noise_level, seed):
    """With probability noise_level, replace the digit with a different one."""
    K = 10
    rng = np.random.RandomState(seed)
    out = labels.copy()
    hit = rng.random_sample(labels.shape[0]) < noise_level
    out[hit] = (labels[hit] + rng.randint(1, K, size=hit.sum())) % K
    return out

def get_noisy_mnist(split='train', size=40000, seed=0, noise_level=0.2, noise_seed=0, training_shuffle_seed=None):
    data, labels = get_mnist_dataset(split, size, seed, training_shuffle_seed)
    labels = corrupt(labels, noise_level=noise_level, seed=noise_seed)
    return data, labels

def get_mnist_dataset(split='train', size=40000, seed=0, training_shuffle_seed=None):
    ...
    if split == 'train':
        if training_shuffle_seed is not None:
            rng = np.random.RandomState(training_shuffle_seed)
            ind = rng.permutation(n1)
            data[:n1, :] = data[ind, :]
            labels[:n1] = labels[ind]
